...
userController.post("/login", async (req, res) => {
  ...
  bcrypt.compare(password, user.password, async (err, result) => {
    ...
    if (result && user.block == false) {
      ...
      let new_unk = await userModel.findOneAndUpdate(
        { email },
        { $set: { count: 0, block: false, logTime: [] } },
        { new: true }
      );
      await new_unk.save();
      return res.send({
      ...
      if (user.block == false) {
        if (user.count == 5) {
          ...
          let new_upd = await userModel.findOneAndUpdate(
            { email },
            { $set: { block: true, logTime: user.logTime } },
            { new: true }
          );
          await new_upd.save();
        } else if (user.count == 4) {
          user.logTime.push(Date.now());
          let new_mod = await userModel.findOneAndUpdate(
            { email },
            { $set: { count: user.count, logTime: user.logTime } },
            { new: true }
          );
          await new_mod.save();
          return res.send({
            message:
              "only one last attempt is left, after that ur block for 24hours ",
          });
        } else {
          user.logTime.push(Date.now());
          // console.log(user)
        ...
        if (unlockTime >= 86400) {
          ...
          if (86400 - unlockTime > 3600) {
            return res.send(
              `Try after ${Math.floor(
                Math.floor((86400 - unlockTime) / 60) / 60
              )} hours`
            );
          } else {
            return res.send(
              `Try after ${Math.floor((3600 - unlockTime) / 60)} mins`
            );
          }
        }
      }
